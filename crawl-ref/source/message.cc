...
struct message_particle
{
    ...
    {
        return false; // no need semicolon
...
void mprf(msg_channel_type channel, int param, const string& msg)
{
    _mpr(msg, channel, param, true, false);
}

void mprf(msg_channel_type channel, const string& msg)
{
    mprf(channel, channel == MSGCH_GOD ? you.religion : 0, msg);
}

void mprf(const string& msg)
{
    mprf(MSGCH_PLAIN, msg);
}
...
static string show_prompt(string prompt)
{
    ...
    return colour_string(jtrans_notrim(prompt), colour_msg(colour));
}
...
void canned_msg(canned_message_type which_message)
{
    switch (which_message)
    {
        ...
        case MSG_CANNOT_DO_YET:
            ...
            break;
        case MSG_OK:
            mprf(MSGCH_PROMPT, jtrans("Okay, then."));
            crawl_state.cancel_cmd_repeat();
            break;
        ...
        case MSG_SPELL_FIZZLES:
            ...
            break;
        case MSG_HUH:
            mprf(MSGCH_EXAMINE_FILTER, jtrans("Huh?"));
            crawl_state.cancel_cmd_repeat();
            break;
        ...
        case MSG_MAGIC_DRAIN:
            mprf(MSGCH_WARN, jtrans("You suddenly feel drained of magical energy!"));
            break;
        case MSG_SOMETHING_IN_WAY:
            mpr(jtrans("There's something in the way."));
        case MSG_CANNOT_SEE:
            mpr(jtrans("You can't see that place."));
            break;
    }
}
