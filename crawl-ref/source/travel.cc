...
{
    string result = branch_name_j(long_name ? branches[branch].longname
                                            : tagged_jtrans("[branch abbrj]", branches[branch].abbrevname));
...
bool explore_discoveries::stop_explore() const
{
    ...
    for (const string &marked : marked_feats)
        mprf(jtransc("Found {something}."), marked.c_str());
